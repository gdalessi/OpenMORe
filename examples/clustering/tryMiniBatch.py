...
X = readCSV(file_options["path_to_file"], "cfdf.csv")
X2 = readCSV(file_options["path_to_file"], file_options["input_file_name"])
...
X_class_tilde = center_scale(X2, mu, sigma)

model_ = MiniBatchKMeans(n_clusters=8, random_state=0, batch_size=int(X_tilde.shape[0]/1))
idxK = model_.fit_predict(X_tilde)
idxClass = model_.predict(X_class_tilde)
...
axes.scatter(mesh[:,0], mesh[:,1], c=idxClass,alpha=0.9, cmap='gnuplot')
axes.set_xlabel('X [m]')
axes.set_ylabel('Y [m]')
plt.show()
