...
model_Kmeans = clustering.KMeans(X, settings)
index = model_Kmeans.fit()

fig = plt.figure()
axes = fig.add_axes([0.2,0.15,0.7,0.7], frameon=True)
sc = axes.scatter(X[:,0], X[:,1], c=index,alpha=0.9, cmap=cmap, edgecolor ='none')
axes.set_title("K-means clustering solution")
axes.set_xlabel('X [-]')
axes.set_ylabel('Y [-]')
#plt.colorbar(sc, extendfrac='auto',spacing='uniform')
cb = plt.colorbar(sc, spacing='uniform', ticks=bounds)
